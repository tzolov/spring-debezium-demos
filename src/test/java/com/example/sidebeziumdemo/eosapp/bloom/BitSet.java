...
public class BitSet {
    ...
    public BitSet(int byteSize) {
        Assert.isTrue(byteSize > 0, "bits size should be greater than 0.");
        this.byteLength = byteSize;
        this.bitLength = byteSize << 3;
    }

    public void setMemorySegment(MemorySegment memorySegment, int offset) {
        Assert.isTrue(memorySegment != null, "MemorySegment can not be null.");
        Assert.isTrue(offset >= 0, "Offset should be positive integer.");
        Assert.isTrue(
    ...
    public void set(int index) {
        Assert.isTrue(index < bitLength && index >= 0, "");
